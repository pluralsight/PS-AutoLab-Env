...
Describe $Computername {
    ...
    Try {
        ...
        It "[$Computername] Should be running standard Server (with desktop experience)" {
        ...
        It "[$Computername] Should have an IP address of 192.168.3.122" {
            $r = Invoke-Command { Get-NetIPAddress -InterfaceAlias Ethernet -AddressFamily IPv4} -session $S1
            $r.IPv4Address | Should Be '192.168.3.122'
        }
